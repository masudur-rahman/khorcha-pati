...
const printStyles = `
  ...
  @page {
    @bottom-right { content: counter(page) " / " counter(pages); font-size: 8px; color: #6B778C; font-family: 'DM Sans', sans-serif; }
  }
...
export default function Statement() {
  ...
  useEffect(() => {
    ...
    fetchReportData(duration).then(data => {
      setReport(data)
      document.title = `Expense Tracker Statement (${fmtShortDate(data.startDate)} — ${fmtShortDate(data.endDate)})`
  ...
  return (
    <div id="statement-root" style={{ background: '#F4F5F7', minHeight: '100vh' }}>
      ...
      <div id="print-controls" style={{
        ...
        <span style={{ color: 'white', fontWeight: 700, fontSize: 13, fontFamily: "'Space Grotesk', sans-serif" }}>Expense Tracker — Statement Preview</span>
      ...
      <div className="page-content" style={{ maxWidth: 900, margin: '0 auto', padding: '70px 20px 40px', background: 'white', minHeight: '100vh', boxShadow: '0 0 30px rgba(0,0,0,0.06)' }}>
          ...
          <thead>
            <tr><td style={{ padding: 0 }}>
              <div className="statement-header" style={{ textAlign: 'center', marginBottom: 28, borderBottom: `3px solid ${ACCENT}`, paddingBottom: 20 }}>
                <h1 style={{ fontSize: 22, fontWeight: 800, color: ACCENT, margin: '0 0 4px', fontFamily: "'Space Grotesk', sans-serif" }}>Expense Tracker Statement</h1>
        ...
        <div style={{ borderTop: '1px solid #DFE1E6', paddingTop: 12, textAlign: 'center', color: '#6B778C', fontSize: 10 }}>
          Generated by Expense Tracker Bot &nbsp;·&nbsp; {fmtFooterTime(report.generatedAt)}
        </div>
...
function StatCard({ label, value, bg, border, color }: { label: string; value: string; bg: string; border: string; color: string }) {
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 12, padding: 16, textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 20, fontWeight: 800, color, margin: 0, fontFamily: "'Space Grotesk', sans-serif" }}>{value}</p>
    </div>
  )
}
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 style={{ fontSize: 13, fontWeight: 700, color: ACCENT, borderLeft: `4px solid ${ACCENT}`, paddingLeft: 10, margin: '0 0 12px', breakAfter: 'avoid', fontFamily: "'Space Grotesk', sans-serif" }}>
      {children}
    </h2>
  )
}
